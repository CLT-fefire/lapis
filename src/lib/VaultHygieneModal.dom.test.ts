...
describe("탭 바", () => {
  it("탭이 아홉이고 각각 숫자를 단다", () => {
    const badges = textOf(".tab .badge");
    expect(badges).toHaveLength(9);
    // 끊긴 링크 1(없는문서) · 고아 4(hub·lonely·dup 둘) · 태그 2묶음 + 모호한 이름 1 = 3
    // 넷째(안 걸린 언급) · 여섯째(미완 작업) · **일곱째(끝났다는데)** · 여덟째(최근 변경)는
    // 아직 안 셌다 — 본문이나 git 을 읽어야 알 수 있고, 탭을 열기 전에는 안 읽는다.
    // 다섯째(속성)는 픽스처에 frontmatter 값이 없어 0이다.
    // 아홉째(오래된 것)는 픽스처에 mtime·date 가 없어 0이다 — **모르는 것은 안 센다.**
    //
    // ⚠️ 일곱째가 `–` 인 것이 요점이다. 부패 감사는 **미완 작업이 읽힌 뒤에야** 셀 수
    //    있으므로, 안 읽고 0 을 띄우면 어긋난 노트를 "없다"고 말하게 된다.
