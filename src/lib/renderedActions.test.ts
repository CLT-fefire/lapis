 ...
 * 🔴 **빈 칸은 방향과 무관하게 뒤로 간다.**
 *
 * `compareCells` 의 주석이 "빈 칸은 **항상** 뒤로"라고 못 박는데, `sortedOrder` 가
 * 비교 결과 전체에 `sign` 을 곱하면서 그 규칙까지 뒤집었다 — 내림차순에서 빈 칸이
 * **맨 위로** 올라온다. 프리뷰에서 표를 직접 정렬해 보고 걸렸다.
 *
 * ⚠️ `tableView.ts` 의 `sortRows` 는 같은 규칙을 **제대로** 지킨다(빈 값을 `sign` 곱하기
 * 전에 처리). 같은 규칙이 두 군데 적혀 있고 한쪽만 틀린, 이 저장소에서 가장 자주 나온 모양이다.
 */
describe("빈 칸은 방향과 무관하게 뒤로", () => {
  const rows = [["가", "2"], ["나", ""], ["다", "1"]];
  ...
  it("내림차순에서도 뒤로", () => {
    // 값 있는 것만 뒤집힌다: 2 → 1, 빈 칸은 그대로 마지막.
    expect(sortedOrder(rows, 1, "desc"), "내림차순에서 빈 칸이 위로 올라온다").toEqual([0, 2, 1]);
  });

  it("빈 칸끼리는 원래 순서", () => {
    const many = [["가", ""], ["나", "5"], ["다", ""]];
    expect(sortedOrder(many, 1, "desc")).toEqual([1, 0, 2]);
  });
});
