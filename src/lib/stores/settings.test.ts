...
describe("mergeSettings", () => {
  it("부분 갱신이 나머지 필드를 보존한다", () => {
    const current = { link_rewrite_backup_keep: 50, mcp_enabled: true };
    expect(mergeSettings(current, { link_rewrite_backup_keep: 30 })).toEqual({
      link_rewrite_backup_keep: 30,
      mcp_enabled: true,
    });
    expect(mergeSettings(current, { mcp_enabled: false })).toEqual({
      link_rewrite_backup_keep: 50,
      mcp_enabled: false,
    });
  });

  it("빈 patch는 현재 값 그대로", () => {
    const current = { link_rewrite_backup_keep: 7, mcp_enabled: true };
...
describe("부분 저장이 다른 필드를 지우지 않는다", () => {
  it("applyBackupKeep이 mcp_enabled를 보존한다", async () => {
    read.mockResolvedValue({ link_rewrite_backup_keep: 50, mcp_enabled: true });
  ...
  it("applyMcpEnabled가 link_rewrite_backup_keep을 보존한다", async () => {
    read.mockResolvedValue({ link_rewrite_backup_keep: 77, mcp_enabled: false });
    await restoreSettings();

    await applyMcpEnabled(true);

    expect(write).toHaveBeenCalledWith({
      link_rewrite_backup_keep: 77,
      mcp_enabled: true,
    });
    expect(get(mcpEnabled)).toBe(true);
  ...
  it("연속 저장이 누적된다 — 앞선 patch가 뒤에서 되돌아가지 않는다", async () => {
    read.mockResolvedValue({ link_rewrite_backup_keep: 20, mcp_enabled: false });
    await restoreSettings();
...
describe("restoreSettings", () => {
  it("읽은 값을 clamp해서 store에 넣는다", async () => {
    read.mockResolvedValue({ link_rewrite_backup_keep: 9999, mcp_enabled: true });
    await restoreSettings();
