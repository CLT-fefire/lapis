<script lang="ts">
  ...
  $effect(() => {
    ...
    // 🔴 **옛 본문을 즉시 비운다.**
    //
    // 읽기는 비동기다. 안 비우면 머리글은 벌써 B 를 말하는데 본문은 A 의 것이 그대로
    // 남아 응답이 와야 바뀐다 — 그 사이가 **틀린 화면**이고, 아무 신호도 없다.
    // 실측으로 걸렸다: `b / b.md ✕  에이의 본문`.
    //
    // ⚠️ 빈 칸이 잠깐 보이는 것은 감수한다. "아직 안 왔다"는 "다른 노트를 보고 있다"보다
    //    훨씬 덜 나쁘다.
    raw = "";
