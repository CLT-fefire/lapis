...
//!
//! ## ⚠️ `searchIndex.ts`의 `computeShardId`와 섞지 말 것
//!
//! 거기도 FNV-1a 32지만 **UTF-16 코드 단위**를 먹인다(`charCodeAt`). 여기는 UTF-8
//! 바이트다. ASCII에서는 우연히 같고 **한글 경로에서 갈린다.** 이름이 같다고 합치면
//! shard 배정이 바뀌어 기존 캐시가 조용히 어긋난다(문서가 다른 shard로 가서 검색에서
//! 사라진다). 그쪽 주석에도 같은 경고를 남겨 뒀다.
//!
//! ## 위협 모델
//!
