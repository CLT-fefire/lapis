...
- **`⌘⇧F` 풀텍스트 검색이 구조 팔도 함께 낸다** — 걸리는 태그와 `doc_kind`/`topic` facet이 본문
  결과 아래에 붙는다. 짧은 질의는 풀텍스트가 원래 못 푸는 문제다: 2어절 R@1이 37.5%로 긴 질의의
  90%대와 격차가 크고, 토크나이저 교체(+1pt)와 결합 4단계화(변화 없음)로 두 번 확인했다. 2어절이
  서른 개 문서에 나오면 그중 하나를 1위로 올릴 근거가 없다 — 원한 게 **그 묶음**일 수 있다.
  종전엔 그 대안이 `⌘K`에만 있었다. 정작 필요한 표면에 없던 것을 옮겨왔다.
