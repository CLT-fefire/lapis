 ...
 * ## ⚠️ 코퍼스가 없으면 못 재는 것들
 *
 * 2026-08-27에 열린 측정 항목 셋을 재려다 **셋 다 이 하네스로는 못 잰다**는 것을 확인했다.
 * 도구를 만들기 전에 코퍼스를 먼저 보라는 뜻이다:
 *
 * | 재려던 것 | 왜 못 재나 |
 * |---|---|
 * | `aliases`를 인덱스 필드로? | 이 vault에 **alias가 있는 노트가 0개**다. `alias` 케이스 종류를 더해도 0건이 나온다 |
 * | `name` 필드를 빼도 되나? | 매칭 수는 **노트 수를 못 넘는다.** 영문 파일명 vault를 대표하지 못한다 |
 * | 한글 bigram 과민 | 그 증상(평균 10,346건 매칭)은 **19,225 노트**에서 관측됐다. 71 노트에서는 재현 자체가 불가능하다 |
 *
 * 셋 다 **하네스가 부족한 게 아니라 코퍼스가 부족하다.** 큰 vault나 영문 vault를 물리면
 * 그때 다시 잴 수 있다 — `--vault`로 대상을 바꿀 수 있게 해 둔 이유이기도 하다.
 */
