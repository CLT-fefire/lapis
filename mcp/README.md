...
`lapis_render`는 **앱이 떠 있어야** 하고 **버전이 3.10.0 이상**이어야 한다. 그 아래는 이 인자를 모르고 조용히 무시해 `app_timeout` 으로만 드러난다 — 앱이 떠 있는데도 그렇다.

⚠️ **그 vault 를 연 창이 없어도 된다.** 아무도 안 받아가면 앱이 창을 하나 띄워 요청받은 vault 를 열고 처리한다(그래서 첫 호출이 느릴 수 있다). 그러고도 안 되면 실패 파일을 쓴다 — 타임아웃이 유일한 신호인 상황을 남기지 않는다.
