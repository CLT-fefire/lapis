...
**Lapis**는 Obsidian에서 영감을 받아 직접 만드는 **1인용 마크다운 지식 워크벤치**다. 이미 `MyProject/docs`에 축적된 306개+ 마크다운 자산(plans, brainstorms, solutions, todos, diagrams)을 백링크·태그·그래프 뷰로 항해하기 위한 도구이며, **즉시 도입보다 배움과 통제권 확보가 1차 목표**다.
...
| **통제권** | 마크다운 + frontmatter는 MyProject 워크플로의 핵심 자산. 외부 도구 의존도 최소화 |
...
- Obsidian이 채워주지 않는 **MyProject 특화 통합** 가능성을 확보 (예: STATE.md 자동 갱신, claude-mem 연동, `/diagram` 산출물 자동 인덱싱)
...
| 15 | 템플릿 시스템 | frontmatter 자동 채움 (MyProject 명명 규칙) |
...
┌─────────────────────────────────────────────────────┐
│              Vault (사용자 마크다운 폴더)           │
│  /Users/Shared/Source/SharedDocs/MyProject 등       │
└─────────────────────────────────────────────────────┘
```
...
목표: **MyProject/docs 폴더를 읽기 전용으로 항해 가능**
...
목표: **MyProject 워크플로 특화 가치 추가**

- [ ] Daily Note + 템플릿 시스템
- [ ] STATE.md pin + 자동 인덱스 블록
- [ ] MyProject 명명 규칙 자동 적용 템플릿
...
| 본업(MyProject)에서 시간을 빼앗김 | 중 | 고 | 주말·저녁만 / 절대 평일 본업 시간 침범 금지 |
...
- [ ] `/Users/Shared/Source/SharedDocs/MyProject` 을 vault로 열고 5초 이내 트리 표시
...
- [ ] 1주일간 Obsidian 없이 Lapis만으로 MyProject 문서 작업 가능
...
- MyProject 컨벤션 차용: `[Phase-N] feat: 백링크 인덱서 1차 구현`
